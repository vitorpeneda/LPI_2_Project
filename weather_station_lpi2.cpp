...
unsigned long nextCalcPressure;	// When we next calc the pressure
unsigned long nextCalcBatt_Level;	// When we next calc the battery level
...
//		rtc.adjust(DateTime("Jul 12 2014","17:05:30"));
...
//*/
  ...
  nextCalcBatt_Level= millis() + MSECS_CALC_BATT_LEVEL;
...
// record current time so we can sample at regular intervals
	...
	if (loopstart >=  nextCalcBatt_Level) {
		Bat_Level();
		nextCalcBatt_Level = loopstart + MSECS_CALC_BATT_LEVEL;
	}
} 
